...
export default function SleepingHome({ navigation }) {
  ...
  const [selectedListID, setselectedListID] = useState(musicCategory[0].id);
  const [isSelected, setSelected] = useState(false);
  const list = listInCategory.filter(
    (itemList) => itemList.idCategory == selectedListID
  );
  console.log(list);
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          ...
          setselectedListID(list[musicCategory[selectedIndex].id]);
        }}
      >

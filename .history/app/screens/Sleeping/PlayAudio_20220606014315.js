...
export default function PlayAudio({ route, navigation }) {
  ...
  async function addToFavorite() {
    if (!didLike) {
      console.log(JSON.stringify(updateAudio));
      await setDoc(
        doc(
          db,
          "favorites",
        ...
        console.log("audio id:" +audio.audio_id);
        const getAudioWithID = query(
          collection(db, "audios"),
          where("audio_id", "==", audio.audio_id)
        );

        const audioList = await getDocs(getAudioWithID);
        console.log(JSON.stringify(audioList.docs.map(doc => doc.data())));
        audioList.forEach(async (doc) => {
          // console.log(JSON.stringify(doc))
          const getAudio = doc(db, 'audios', doc.data().audio_id);
          console.log(`doc.data().audio_id ${doc.data().audio_id}`);
          console.log(`doc.audio_id ${doc.audio_id}`);
          await updateDoc(getAudio, {
           total_likes: 1
          });
        });
      });

      //  const au = audioList.docs[0].data();
      // setUpdateAudio(audioList.docs[0]);
      //   console.log(JSON.stringify(audioList.docs[0].data()));
      //   const currentTotalLike = audioList.docs[0].data().total_likes;
      //   console.log("currentTotalLike: " + currentTotalLike);
      //   await updateDoc(audioList.docs[0].data(), {
      //     total_likes: 1,
      //   });
      // });
    } else {

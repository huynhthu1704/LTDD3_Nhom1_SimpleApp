...
export function AudioItem({ item }) {
  ...
  return (
    <TouchableOpacity
      ...
      onPress = {() => setSelected(!isSelected)}
    >
    <Image style = {{width : 80, height : 80}} source={{uri : "http://khoack.tdc.edu.vn/wp-content/uploads/2017/01/Logo-Cao-dang1.jpg"}}/>
      <Text>{item.name}</Text>
...
export default function SleepingHome() {
  return (
    <View style = {{backgroundColor : COLORS.purple, flex : 1}}>
      ...
      {/* Categoty list */}
      <View>
        <FlatList data = {audio} renderItem = {({item}) => <AudioItem item = {item} />}/>
      </View>
    </View>
  );
}

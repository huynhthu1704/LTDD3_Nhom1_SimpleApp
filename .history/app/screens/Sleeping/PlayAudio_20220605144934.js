...
export default async function PlayAudio({ route, navigation }) {
  ...
  useEffect(async () => {
    ...
    const audioIsLiked = favorites.find((item) => item.audio_id == audio.id && item.user_id == currentUserID)
  ...
  async function addToFavorite() {
    if (!didLike) {
      ...
      console.log(size);
      await setDoc(doc(db, "favorites", `user${currentUserID}-audio${audio?.audio_id}`), {
